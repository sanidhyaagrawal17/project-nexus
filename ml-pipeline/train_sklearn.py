...
def run_sklearn_pipeline():
    print('=== Initializing Project Nexus Sklearn Pipeline ===')
    ...
    print('\n[*] Phase 3: Training RandomForest classifier...')
    ...
    print('\n[*] Phase 3b: Training standby RandomForest classifier...')
    standby_rf_model = RandomForestClassifier(
        n_estimators=200,
        max_depth=6,
        random_state=99,
        n_jobs=-1,
    )
    standby_rf_model.fit(X_train_balanced, y_train_balanced)

    calibrated_model = rf_model
    standby_calibrated_model = standby_rf_model
    calibration_available = False
    standby_calibration_available = False
    try:
        calibration_model = CalibratedClassifierCV(estimator=rf_model, method='sigmoid', cv='prefit')
    ...
    try:
        standby_calibration_model = CalibratedClassifierCV(estimator=standby_rf_model, method='sigmoid', cv='prefit')
    ...
    raw_validation_probabilities = rf_model.predict_proba(X_calib_enhanced)[:, 1]
    raw_standby_validation_probabilities = standby_rf_model.predict_proba(X_calib_enhanced)[:, 1]
    ...
    joblib.dump(rf_model, os.path.join(model_dir, 'nexus_xgboost.pkl'))
    joblib.dump(standby_rf_model, os.path.join(model_dir, 'nexus_xgboost_standby.pkl'))
